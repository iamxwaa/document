...
HDFS --> Configs --> Advanced,修改Custom core-site,添加
...
make[2]: *** [/data/hue/hue/desktop/core/build/python-ldap-2.3.13/egg.stamp] Error 1
make[2]: Leaving directory `/data/hue/hue/desktop/core'
make[1]: *** [.recursive-env-install/core] Error 2
make[1]: Leaving directory `/data/hue/hue/desktop'
...
make[2]: *** [/data/hue/hue/desktop/core/build/MySQL-python-1.2.5/egg.stamp] Error 1
make[2]: Leaving directory `/data/hue/hue/desktop/core'
make[1]: *** [.recursive-env-install/core] Error 2
make[1]: Leaving directory `/data/hue/hue/desktop'
make: *** [desktop] Error 2
...
```bash
java.security.AccessControlException: User [hue] not defined as proxyuser
...
```bash
curl -O https://raw.githubusercontent.com/pypa/get-pip/master/3.2/get-pip.py
python get-pip.py
...
ln -s /data/node-v10.15.3-linux-x64/bin/npm /usr/local/bin/
ln -s /data/node-v10.15.3-linux-x64/bin/node /usr/local/bin/
...
```bash
pip install ipython==5.2.0;pip install ipdb==0.10.3;pip install nose==1.3.7;pip install coverage==4.4.2;pip install nosetty==0.4;pip install werkzeug==0.14.1;pip install windmill==1.6;pip install astroid==1.5.3;pip install isort==4.2.5;pip install six==1.10.0
```
...
services:
  cloudera:
      ...
      - "8888:8888"
      volumes:
        - /data/docker/hue/conf:/usr/share/hue/desktop/conf
        - /data/docker/hue/bigdata/hadoop/3.0.1.0-187/0:/etc/hadoop/conf
        - /data/docker/hue/bigdata/hbase/3.0.1.0-187/0:/etc/hbase/conf
        - /data/docker/hue/bigdata/hive/3.0.1.0-187/0:/etc/hive/conf
        - /data/docker/hue/bigdata/oozie/3.0.1.0-187/0:/etc/oozie/conf
        - /data/docker/hue/bigdata/spark2/3.0.1.0-187/0:/etc/spark2/conf
        - /data/docker/hue/bigdata/spark2/3.0.1.0-187/0:/etc/spark/conf
        - /data/docker/hue/bigdata/zookeeper/3.0.1.0-187/0:/etc/zookeeper/conf

      extra_hosts:
        - "database:192.168.119.203"
        - "vrv203:192.168.119.203"
        - "vrv204:192.168.119.204"
        - "vrv205:192.168.119.205"
        - "vrv206:192.168.119.206"
        - "vrv207:192.168.119.207"
volumes:
  data:
```
...
[desktop]
  ...
  # e.g.: pig,zookeeper
  app_blacklist=pig,impala,search,sqoop
...
[notebook]
  ...
  [[interpreters]]
    ...
    [[[mysql]]]
      ...
      options='{"url": "mysql://root:root@192.168.119.208:3306/vap_audit"}'
...
[hadoop]
  ...
  [[hdfs_clusters]]
    ...
    [[[default]]]
      ...
      webhdfs_url=http://vrv203:50070/webhdfs/v1
  ...
  [[yarn_clusters]]
    ...
    [[[default]]]
      ...
      resourcemanager_api_url=http://vrv203:8088

      # URL of the ProxyServer API
      ## proxy_api_url=http://localhost:8088

      # URL of the HistoryServer API
      history_server_api_url=http://vrv203:19888

      # URL of the Spark History Server
      spark_history_server_url=http://vrv207:18081
...
[beeswax]
  ...
  # hive_server_host=vrv203:2181,vrv204:2181,vrv205:2181,vrv206:2181,vrv207:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2
  hive_server_host=vrv206
...
[spark]
  # The Livy Server URL.
  livy_server_url=http://vrv205:8999
...
[hbase]
  ...
  hbase_clusters=(Cluster|vrv206:9090)
...
[zookeeper]
  ...
  [[clusters]]
    ...
    [[[default]]]
      ...
      host_ports=vrv203:2181,vrv204:2181,vrv205:2181,vrv206:2181,vrv207:2181
...
[liboozie]
  ...
  oozie_url=http://vrv207:11000/oozie
